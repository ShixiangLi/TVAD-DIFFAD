...
class CustomTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # --- Load Current Features ---
        # Path: self.data_path_base / current_features / test / image_type_folder / frame_0001.npy
        npy_filename_cf = file_name_only.replace('.png', '.npy') # Adjust for other extensions
        ...
        if os.path.exists(current_feature_path):
            ...
            current_features_data = np.zeros(3, dtype=np.float32) # Placeholder (3,)
...
class CustomTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # --- Load Current Features ---
        # Path: self.data_path_base / current_features / train / good / frame_xxxx.npy
        file_name_only_train = os.path.basename(image_path)
        npy_filename_cf_train = file_name_only_train.replace('.png', '.npy') # Adjust extensions
        ...
        if os.path.exists(current_feature_path_train):
            ...
            current_features_data_train = np.zeros(3, dtype=np.float32) # Placeholder (3,)
